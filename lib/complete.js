...
/**
 * Get rc filename on Darwin
 */


function getRc() {
  var possibleRcFiles = ['.bash_profile','.bash_login','.profile']
    , i
    , len;
  
  //check all possible rc files
  //return if file exists
  for(i = 0, len = possibleRcFiles.length;i<len;i++){
      var profile = join(home, possibleRcFiles[i]);
      try {
        fs.statSync(profile);
        return possibleRcFiles[i];
      }  
      catch(e) {}
  }
